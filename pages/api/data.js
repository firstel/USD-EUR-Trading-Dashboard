  ...
  try {
    ...
    if (!timeSeries) {
      ...
      const mockData = Array.from({ length: 30 }, (_, i) => ({
        timestamp: new Date(
          Date.now() - i * 60 * 60 * 1000 // Newest first
        ).toISOString(),
    ...
    // Convert EUR/USD to USD/EUR and format - NEWEST FIRST
    ...
    // Already in newest first order from Alpha Vantage

    res.status(200).json(result);
  } catch (error) {
    // Fallback to mock data on error - NEWEST FIRST
    const mockData = Array.from({ length: 30 }, (_, i) => ({
