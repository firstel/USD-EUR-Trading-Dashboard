...
async function findBestStrategy() {
  const strategies = ['MACD', 'RSI', 'MA', 'HYBRID']; // Added HYBRID
  const performances = {};
...
async function sendSignalAlert(strategy, signalData, latestSignal) {
  const latestPrice = signalData[0]?.USD_EUR; // First item is newest
...
export default async function handler(req, res) {
  try {
    // Find best performing strategy (including HYBRID)
    ...
    // Get latest signal (first item since newest first)
    const latestSignal = signalData[0]?.Signal;
    const previousSignal = signalData[1]?.Signal;
